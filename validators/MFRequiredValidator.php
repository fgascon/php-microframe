<?php
/**
 * MFRequiredValidator class file.
 *
...
/**
 * MFRequiredValidator validates that the specified attribute does not have null or empty value.
 *
 * When using the {@link message} property to define a custom error message, the message
 * may contain additional placeholders that will be replaced with the actual content. In addition
 * to the "{attribute}" placeholder, recognized by all validators (see {@link MFValidator}),
 * MFRequiredValidator allows for the following placeholders to be specified:
 * <ul>
 ...
 * @since 1.0
 */
class MFRequiredValidator extends MFValidator
{
	/**
	 ...
	 * @param MFModel $object the object being validated
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		if($this->requiredValue!==null)
		{
			...
			{
				$message=$this->message!==null?$this->message:MF::t('core','{attribute} must be {value}.',
		...
		{
			$message=$this->message!==null?$this->message:MF::t('core','{attribute} cannot be blank.');
			$this->addError($object,$attribute,$message);
		}
	}
	...
	public function clientValidateAttribute($object,$attribute)
	{
		...
		{
			if($message===null)
				$message=MF::t('core','{attribute} must be {value}.');
...
";
		...
		{
			if($message===null)
				$message=MF::t('core','{attribute} cannot be blank.');
			$message=strtr($message, array(
				'{attribute}'=>$object->getAttributeLabel($attribute),
			));
			if($this->trim)
