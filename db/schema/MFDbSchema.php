...
/**
 * MFDbSchema is the base class for retrieving metadata information.
 *
 * @property MFDbConnection $dbConnection Database connection. The connection is active.
 * @property array $tables The metadata for all tables in the database.
 * Each array element is an instance of {@link MFDbTableSchema} (or its child class).
 * The array keys are table names.
 * @property array $tableNames All table names in the database.
 * @property MFDbCommandBuilder $commandBuilder The SQL command builder for this connection.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
...
abstract class MFDbSchema extends CComponent
{
	 ...
	 * @return MFDbTableSchema driver dependent table metadata, null if the table does not exist.
	 */
	abstract protected function loadTable($name);

	/**
	 * Constructor.
	 * @param MFDbConnection $conn database connection.
	 */
	...
	/**
	 * @return MFDbConnection database connection. The connection is active.
	 */
	 ...
	 * @return MFDbTableSchema table metadata. Null if the named table does not exist.
	 */
	public function getTable($name,$refresh=false)
	{
		...
		else
		{
			...
			if(!isset($this->_cacheExclude[$name]) && ($duration=$this->_connection->schemaCachingDuration)>0 && $this->_connection->schemaCacheID!==false && ($cache=MF::app()->getComponent($this->_connection->schemaCacheID))!==null)
			{
				$key='mf:dbschema'.$this->_connection->connectionString.':'.$this->_connection->username.':'.$name;
				$table=$cache->get($key);
	...
	/**
	 * @return MFDbCommandBuilder the SQL command builder for this connection.
	 */
	...
	{
		if(($duration=$this->_connection->schemaCachingDuration)>0 && $this->_connection->schemaCacheID!==false && ($cache=MF::app()->getComponent($this->_connection->schemaCacheID))!==null)
		{
	 ...
	 * @param MFDbTableSchema $table the table schema whose primary key sequence will be reset
	 ...
	 * @return MFDbCommandBuilder command builder instance
	 */
	protected function createCommandBuilder()
	{
		return new MFDbCommandBuilder($this);
	}
	 ...
	 * @param string $schema the schema of the tables. Defaults to empty string, meaning the current or default schema.
	 * If not empty, the returned table names will be prefixed with the schema name.
	 * @throws MFDbException if current schema does not support fetching all table names
	 * @return array all table names in the database.
	 */
	protected function findTableNames($schema='')
	{
		throw new MFDbException(MF::t('core','{class} does not support fetching all table names.',
