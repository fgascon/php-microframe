...
/**
 * MFMysqlSchema is the class for retrieving metadata information from a MySQL database (version 4.1.x and 5.x).
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db.schema.mysql
 * @since 1.0
 */
class MFMysqlSchema extends MFDbSchema
{
	/**
	 ...
	 * @param string $name table name
	 * @return MFMysqlTableSchema driver dependent table metadata. Null if the table does not exist.
	 */
	protected function loadTable($name)
	{
		$table=new MFMysqlTableSchema;
	 ...
	 * @param MFMysqlTableSchema $table the table instance
	 ...
	 * @param MFMysqlTableSchema $table the table metadata
	 ...
	 * @return MFDbColumnSchema normalized column metadata
	 */
	protected function createColumn($column)
	{
		$c=new MFMysqlColumnSchema;
		$c->name=$column['Field'];
	...
	{
		return new MFMysqlCommandBuilder($this);
	}
	...
	public function renameColumn($table, $name, $newName)
	{
		...
		if($row===false)
			throw new MFDbException(MF::t('core','Unable to find "{column}" in table "{table}".',array('{column}'=>$name,'{table}'=>$table)));
