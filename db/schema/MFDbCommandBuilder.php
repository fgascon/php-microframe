...
/**
 * MFDbCommandBuilder provides basic methods to create query commands for tables.
 *
 * @property MFDbConnection $dbConnection Database connection.
 * @property MFDbSchema $schema The schema for this command builder.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
...
class MFDbCommandBuilder extends CComponent
{
	...
	/**
	 * @param MFDbSchema $schema the schema for this command builder
	 */
	...
	/**
	 * @return MFDbConnection database connection.
	 */
	...
	/**
	 * @return MFDbSchema the schema for this command builder.
	 */
	 ...
	 * @param mixed $table the table schema ({@link MFDbTableSchema}) or the table name (string).
	 ...
	 * @return MFDbCommand query command.
	 */
	 ...
	 * @param mixed $table the table schema ({@link MFDbTableSchema}) or the table name (string).
	 * @param MFDbCriteria $criteria the query criteria
	 * @return MFDbCommand delete command.
	 */
	 ...
	 * @return MFDbCommand insert command
	 */
	 ...
	 * @return MFDbCommand multiple insert command
	 * @since 1.1.14
	 */
	 ...
	 * @return MFDbCommand multiple insert command
	 * @throws MFDbException if $data is empty.
	 */
	protected function composeMultipleInsertCommand($table,array $data,array $templates=array())
	{
		if (empty($data))
			throw new MFDbException(MF::t('core','Can not generate multiple insert command with empty data set.'));
		...
		foreach($data as $rowKey=>$rowData)
		{
			...
			foreach($columns as $columnName)
			{
				...
				$columnValue=array_key_exists($columnName,$rowData) ? $rowData[$columnName] : new MFDbExpression('NULL');
				if($columnValue instanceof MFDbExpression)
				{
	 ...
	 * @param MFDbCriteria $criteria the query criteria
	 * @throws MFDbException if no columns are being updated for the given table
	 * @return MFDbCommand update command.
	 */
	public function createUpdateCommand($table,$data,$criteria)
	{
		...
		if($fields===array())
			throw new MFDbException(MF::t('core','No columns are being updated for table "{table}".',
	 ...
	 * @param MFDbCriteria $criteria the query criteria
	 * @throws MFDbException if no columns are being updated for the given table
	 * @return MFDbCommand the created command
	 */
	public function createUpdateCounterCommand($table,$counters,$criteria)
	{
		...
		if($fields!==array())
		{
			...
			throw new MFDbException(MF::t('core','No counter columns are being updated for table "{table}".',
				array('{table}'=>$table->name)));
	}
	 ...
	 * @param MFDbCommand $command database command
	 ...
	 * @return MFDbCriteria the created query criteria
	 * @throws CException if the condition is not string, array and MFDbCriteria
	 */
	public function createCriteria($condition='',$params=array())
	{
		if(is_array($condition))
			$criteria=new MFDbCriteria($condition);
		elseif($condition instanceof MFDbCriteria)
			$criteria=clone $condition;
		else
		{
			$criteria=new MFDbCriteria;
	 ...
	 * If an array, it is treated as the initial values for constructing a {@link MFDbCriteria};
	 * Otherwise, it should be an instance of {@link MFDbCriteria}.
	 * @param array $params parameters to be bound to an SQL statement.
	 ...
	 * In other cases, please use {@link MFDbCriteria::params} to set parameters.
	 * @param string $prefix column prefix (ended with dot). If null, it will be the table name
	 * @throws MFDbException if specified column is not found in given table
	 * @return MFDbCriteria the created query criteria
	 */
	public function createColumnCriteria($table,$columns,$condition='',$params=array(),$prefix=null)
	{
		...
		foreach($columns as $name=>$value)
		{
			if(($column=$table->getColumn($name))!==null)
			{
				...
				throw new MFDbException(MF::t('core','Table "{table}" does not have a column named "{column}".',
	...
	public function createInCondition($table,$columnName,$values,$prefix=null)
	{
		...
		elseif(is_array($columnName)) // composite key: $values=array(array('pk1'=>'v1','pk2'=>'v2'),array(...))
		{
			foreach($columnName as $name)
			{
				...
				for($i=0;$i<$n;++$i)
				{
					if(isset($values[$i][$name]))
					{
						...
						throw new MFDbException(MF::t('core','The value for the column "{column}" is not supplied when querying the table "{table}".',
							array('{table}'=>$table->name,'{column}'=>$name)));
				}
			}
			if(count($values)===1)
			...
			throw new MFDbException(MF::t('core','Column name must be either a string or an array.'));
	}

	/**
	 * Generates the expression for selecting rows with specified composite key values.
	 * @param MFDbTableSchema $table the table schema
	 * @param array $values list of primary key values to be selected within
