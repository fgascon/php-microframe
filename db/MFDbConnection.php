...
/**
 * MFDbConnection represents a connection to a database.
 *
 * MFDbConnection works together with {@link MFDbCommand}, {@link MFDbDataReader}
 * and {@link MFDbTransaction} to provide data access to various DBMS
 ...
 *
 * The following example shows how to create a MFDbConnection instance and establish
 * the actual connection:
 * <pre>
 * $connection=new MFDbConnection($dsn,$username,$password);
 ...
 * </pre>
 *
 * MFDbConnection also provides a set of methods to support setting and querying
 * of certain DBMS attributes, such as {@link getNullConversion nullConversion}.
 *
 * Since MFDbConnection implements the interface IApplicationComponent, it can
 ...
 * @property MFDbTransaction $currentTransaction The currently active transaction. Null if no active transaction.
 * @property MFDbSchema $schema The database schema for the current connection.
 * @property MFDbCommandBuilder $commandBuilder The command builder.
 ...
 * @since 1.0
 */
class MFDbConnection extends MFComponent
{
	/**
	 ...
	 * effective when the MFDbConnection object is used as an application component.
	 */
	 ...
	 * By setting this property, any token like '{{tableName}}' in {@link MFDbCommand::text} will
	 ...
	 * when the MFDbConnection is used as an application component.
	...
	protected function open()
	{
		if($this->_pdo===null)
		{
			if(empty($this->connectionString))
				throw new MFDbException('MFDbConnection.connectionString cannot be empty.');
			try
			{
				MF::trace('Opening DB connection','system.db.MFDbConnection');
			...
			catch(PDOException $e)
			{
				if(MF_DEBUG)
				{
					throw new MFDbException('MFDbConnection failed to open the DB connection: '.
						$e->getMessage(),(int)$e->getCode(),$e->errorInfo);
				}
				else
				{
					MF::log($e->getMessage(),CLogger::LEVEL_ERROR,'exception.MFDbException');
					throw new MFDbException('MFDbConnection failed to open the DB connection.',(int)$e->getCode(),$e->errorInfo);
				}
			}
		}
	}
	...
	{
		MF::trace('Closing DB connection','system.db.MFDbConnection');
		$this->_pdo=null;
		$this->_active=false;
		$this->_schema=null;
	}
	 ...
	 * @throws MFDbException when failed to open DB connection
	...
	protected function createPdoInstance()
	{
		...
		if(!class_exists($pdoClass))
			throw new MFDbException(MF::t('core','MFDbConnection is unable to find PDO class "{className}". Make sure PDO is installed correctly.',
				array('{className}'=>$pdoClass)));

		@$instance=new $pdoClass($this->connectionString,$this->username,$this->password,$this->_attributes);

		if(!$instance)
			throw new MFDbException(MF::t('core','MFDbConnection failed to open the DB connection.'));
	 ...
	 * or an array representing different fragments of a SQL statement. Please refer to {@link MFDbCommand::__construct}
	 * for more details about how to pass an array as the query. If this parameter is not given,
	 * you will have to call query builder methods of {@link MFDbCommand} to build the DB query.
	 * @return MFDbCommand the DB command
	 */
	public function createCommand($query=null)
	{
		$this->setActive(true);
		return new MFDbCommand($this,$query);
	}

	/**
	 * Returns the currently active transaction.
	 * @return MFDbTransaction the currently active transaction. Null if no active transaction.
	 */
	 ...
	 * @return MFDbTransaction the transaction initiated
	 */
	public function beginTransaction()
	{
		MF::trace('Starting transaction','system.db.MFDbConnection');
		$this->setActive(true);
		$this->_pdo->beginTransaction();
		return $this->_transaction=new MFDbTransaction($this);
	}

	/**
	 * Returns the database schema for the current connection
	 * @throws MFDbException if MFDbConnection does not support reading schema for specified database driver
	 * @return MFDbSchema the database schema for the current connection
	 */
	public function getSchema()
	{
		...
		else
		{
			...
			{
			    $className = $this->driverMap[$driver];
				return $this->_schema=new $className($this);
			}
			else
				throw new MFDbException(MF::t('core','MFDbConnection does not support reading schema for {driver} database.',
					array('{driver}'=>$driver)));
		}
	}
	 ...
	 * @return MFDbCommandBuilder the command builder
	 */
	...
	/*public function getStats()
	{
		...
		$timings=$logger->getProfilingResults(null,'system.db.MFDbCommand.query');
		$count=count($timings);
		$time=array_sum($timings);
		$timings=$logger->getProfilingResults(null,'system.db.MFDbCommand.execute');
		$count+=count($timings);
		$time+=array_sum($timings);
