<?php
/**
 * This file contains the MFDbCommand class.
 *
...
/**
 * MFDbCommand represents an SQL statement to execute against a database.
 *
 * It is usually created by calling {@link MFDbConnection::createCommand}.
 ...
 * can be accessed via the returned {@link MFDbDataReader}.
 *
 * MFDbCommand supports SQL statement preparation and parameter binding.
 ...
 *
 * Starting from version 1.1.6, MFDbCommand can also be used as a query builder
 ...
 * @property MFDbConnection $connection The connection associated with this command.
 ...
 * @since 1.0
 */
class MFDbCommand extends MFComponent
{
	/**
	...
	/**
	 * Constructor.
	 * @param MFDbConnection $connection the database connection
	 ...
	 * Since 1.1.7 it is possible to use a specific mode of data fetching by setting
 	 ...
 	 * for more details.
	 */
	public function __construct(MFDbConnection $connection,$query=null)
	{
	...
	/**
	 * @return MFDbConnection the connection associated with this command
	 */
	 ...
	 * automatically.
	 * @throws MFDbException if MFDbCommand failed to prepare the SQL statement
	 */
	public function prepare()
	{
		if($this->_statement==null)
		{
			...
			{
				MF::log('Error in preparing SQL: '.$this->getText(),'error','system.db.MFDbCommand');
				$errorInfo=$e instanceof PDOException ? $e->errorInfo : null;
				throw new MFDbException(MF::t('core','MFDbCommand failed to prepare the SQL statement: {error}',
	 ...
	 * @throws MFDbException execution failed
	 */
	public function execute($params=array())
	{
		if($this->_connection->enableParamLogging && ($pars=array_merge($this->_paramLog,$params))!==array())
		{
			...
			$par='';
		MF::trace('Executing SQL: '.$this->getText().$par,'system.db.MFDbCommand');
		try
		{
			if($this->_connection->enableProfiling)
				MF::beginProfile('system.db.MFDbCommand.execute('.$this->getText().$par.')','system.db.MFDbCommand.execute');
			...
			if($this->_connection->enableProfiling)
				MF::endProfile('system.db.MFDbCommand.execute('.$this->getText().$par.')','system.db.MFDbCommand.execute');
		...
		catch(Exception $e)
		{
			...
			MF::log(MF::t('core','MFDbCommand::execute() failed: {error}. The SQL statement executed was: {sql}.',
				array('{error}'=>$message, '{sql}'=>$this->getText().$par)),'error','system.db.MFDbCommand');

			if(MF_DEBUG)
				$message.='. The SQL statement executed was: '.$this->getText().$par;

			throw new MFDbException(MF::t('core','MFDbCommand failed to execute the SQL statement: {error}',
	 ...
	 * Please also note that all values are treated as strings in this case, if you need them to be handled as
	 * their real data types, you have to use {@link bindParam} or {@link bindValue} instead.
	 * @return MFDbDataReader the reader object for fetching the query result
	 ...
	 * Please also note that all values are treated as strings in this case, if you need them to be handled as
	 * their real data types, you have to use {@link bindParam} or {@link bindValue} instead.
	 * @throws MFDbException if MFDbCommand failed to execute the SQL statement
	...
	private function queryInternal($method,$mode,$params=array())
	{
		...
		MF::trace('Querying SQL: '.$this->getText().$par,'system.db.MFDbCommand');

		if($this->_connection->queryCachingCount>0 && $method!==''
				&& $this->_connection->queryCachingDuration>0
				&& $this->_connection->queryCacheID!==false
				&& ($cache=MF::app()->getComponent($this->_connection->queryCacheID))!==null)
		{
			$this->_connection->queryCachingCount--;
			$cacheKey='mf:dbquery'.':'.$method.':'.$this->_connection->connectionString.':'.$this->_connection->username;
			$cacheKey.=':'.$this->getText().':'.serialize(array_merge($this->_paramLog,$params));
			if(($result=$cache->get($cacheKey))!==false)
			{
				MF::trace('Query result found in cache','system.db.MFDbCommand');
				return $result[0];
			}
		}
		...
		{
			if($this->_connection->enableProfiling)
				MF::beginProfile('system.db.MFDbCommand.query('.$this->getText().$par.')','system.db.MFDbCommand.query');
			...
			if($method==='')
				$result=new MFDbDataReader($this);
			else
			{
				$mode=(array)$mode;
			...
			if($this->_connection->enableProfiling)
				MF::endProfile('system.db.MFDbCommand.query('.$this->getText().$par.')','system.db.MFDbCommand.query');
		...
		catch(Exception $e)
		{
			...
			$errorInfo=$e instanceof PDOException ? $e->errorInfo : null;
			$message=$e->getMessage();
			MF::log(MF::t('core','MFDbCommand::{method}() failed: {error}. The SQL statement executed was: {sql}.',
				array('{method}'=>$method, '{error}'=>$message, '{sql}'=>$this->getText().$par)),'error','system.db.MFDbCommand');
	 ...
	 * @throws MFDbException if "from" key is not present in given query parameter
	 ...
	 * @return MFDbCommand the command object itself
	 * @since 1.1.6
	 */
	...
	public function insert($table, $columns)
	{
		...
		foreach($columns as $name=>$value)
		{
			...
			if($value instanceof MFDbExpression)
			{
	 ...
	 * @throws MFDbException if unknown operator is used
	...
	private function processConditions($conditions)
	{
		...
		throw new MFDbException(MF::t('core', 'Unknown operator "{operator}".', array('{operator}'=>$operator)));
	}
