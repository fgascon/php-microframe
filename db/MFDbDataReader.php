...
/**
 * MFDbDataReader represents a forward-only stream of rows from a query result set.
 *
 * To read the current row of data, call {@link read}. The method {@link readAll}
 * returns all the rows in a single array.
 *
 * One can also retrieve the rows of data in MFDbDataReader by using foreach:
 * <pre>
 * foreach($reader as $row)
 *     // $row represents a row of data
 * </pre>
 * Since MFDbDataReader is a forward-only stream, you can only traverse it once.
 *
 ...
 * @since 1.0
 */
class MFDbDataReader extends MFComponent implements Iterator, Countable
{
	private $_statement;
	...
	/**
	 * Constructor.
	 * @param MFDbCommand $command the command generating the query result
	 */
	public function __construct(MFDbCommand $command)
	{
	...
	public function rewind()
	{
		if($this->_index<0)
		{
			...
			throw new MFDbException(MF::t('core','MFDbDataReader cannot rewind. It is a forward-only reader.'));
	}
