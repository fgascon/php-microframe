...
/**
 * MFActiveRecord is the base class for classes representing relational data.
 *
 ...
 * about this class.
 *
 * @property MFDbCriteria $dbCriteria The query criteria that is associated with this model.
 * This criteria is mainly used by {@link scopes named scope} feature to accumulate
 * different criteria specifications.
 * @property MFActiveRecordMetaData $metaData The meta for this AR class.
 * @property MFDbConnection $dbConnection The database connection used by active record.
 ...
 * @since 1.0
 */
abstract class MFActiveRecord extends MFModel
{
	const BELONGS_TO='MFBelongsToRelation';
	const HAS_ONE='MFHasOneRelation';
	const HAS_MANY='MFHasManyRelation';
	const MANY_MANY='MFManyManyRelation';
	const STAT='MFStatRelation';

	/**
	 * @var MFDbConnection the default database connection for all active record classes.
	...
	protected $databaseName = 'default';


	/**
	 * Constructor.
	 * @param string $scenario scenario name. See {@link MFModel::scenario} for more details about this parameter.
	 ...
	 * This is a shortcut method to {@link MFDbConnection::cache()}.
	 ...
	 * @param mixed $params array or MFDbCriteria object with additional parameters that customize the query conditions as specified in the relation declaration.
	 ...
	 * @throws MFDbException if the relation is not specified in {@link relations}.
	 */
	public function getRelated($name,$refresh=false,$params=array())
	{
		...
		if(!isset($md->relations[$name]))
			throw new MFDbException(MF::t('core','{class} does not have relation "{name}".',
				array('{class}'=>get_class($this), '{name}'=>$name)));

		MF::trace('lazy loading '.get_class($this).'.'.$name,'system.db.ar.MFActiveRecord');
		$relation=$md->relations[$name];
		if($this->getIsNewRecord() && !$refresh && ($relation instanceof MFHasOneRelation || $relation instanceof MFHasManyRelation))
			return $relation instanceof MFHasOneRelation ? null : array();
		...
		if($params!==array()) // dynamic query
		{
			...
			if($params instanceof MFDbCriteria)
		...
		{
			if($relation instanceof MFHasManyRelation)
				$this->_related[$name]=array();
			elseif($relation instanceof MFStatRelation)
	 ...
	 * @return MFDbCriteria the query criteria that is associated with this model.
	 * This criteria is mainly used by {@link scopes named scope} feature to accumulate
	 * different criteria specifications.
	 */
	 ...
	 * @param MFDbCriteria $criteria the query criteria
	 * @since 1.1.3
	 */
	 ...
	 * @return MFActiveRecordMetaData the meta for this AR class.
	 */
	public function getMetaData()
	{
		...
		if(!array_key_exists($className,self::$_md))
		{
			...
			self::$_md[$className]=new MFActiveRecordMetaData($this);
		}
	 ...
	 * as an array whose keys must be properties of {@link MFDbCriteria}.
	 */
		...
		elseif(strpos($attribute,'.')!==false)
		{
			...
			foreach($segs as $seg)
			{
				...
				if(isset($relations[$seg]))
					$model=MFActiveRecord::model($relations[$seg]->className);
				else
					break;
			}
	 ...
	 * @throws MFDbException if "db" application component is not defined
	 * @return MFDbConnection the database connection used by active record.
	 */
	 ...
	 * @param string $name the relation name
	 * @return MFActiveRelation the named relation declared for this AR class. Null if the relation does not exist.
	 */
	...
	/**
	 * Do not call this method. This method is used internally by {@link MFActiveFinder} to populate
	...
	public function addRelatedRecord($name,$record,$index)
	{
		if($index!==false)
		{
			if(!isset($this->_related[$name]))
				$this->_related[$name]=array();
			if($record instanceof MFActiveRecord)
			{
				if($index===true)
	 ...
	 * By setting {@link MFModelEvent::isValid} to be false, the normal {@link save()} process will be stopped.
	 * @param MFModelEvent $event the event parameter
	 */
	 ...
	 * By setting {@link MFModelEvent::isValid} to be false, the normal {@link delete()} process will be stopped.
	 * @param MFModelEvent $event the event parameter
	 */
	 ...
	 * This can be either a call to MFActiveRecords find methods or a find call
	 ...
	 * @return MFActiveFinder active finder for the operation
	 *
	 * @since 1.1.14
	 */
	public function getActiveFinder($with)
	{
		return new MFActiveFinder($this,$with);
	}
	 ...
	 * @throws MFDbException if the record is not new
	 */
	public function insert($attributes=null)
	{
		if(!$this->getIsNewRecord())
			throw new MFDbException(MF::t('core','The active record cannot be inserted to database because it is not new.'));
		if($this->beforeSave())
		{
			MF::trace(get_class($this).'.insert()','system.db.ar.MFActiveRecord');
	 ...
	 * @throws MFDbException if the record is new
	 */
	public function update($attributes=null)
	{
		if($this->getIsNewRecord())
			throw new MFDbException(MF::t('core','The active record cannot be updated because it is new.'));
		if($this->beforeSave())
		{
			MF::trace(get_class($this).'.update()','system.db.ar.MFActiveRecord');
			if($this->_pk===null)
				$this->_pk=$this->getPrimaryKey();
	...
	public function saveAttributes($attributes)
	{
		if(!$this->getIsNewRecord())
		{
			MF::trace(get_class($this).'.saveAttributes()','system.db.ar.MFActiveRecord');
			$values=array();
	...
	{
		MF::trace(get_class($this).'.saveCounters()','system.db.ar.MFActiveRecord');
		$builder=$this->getCommandBuilder();
		$table=$this->getTableSchema();
	...
	public function delete()
	{
		if(!$this->getIsNewRecord())
		{
			MF::trace(get_class($this).'.delete()','system.db.ar.MFActiveRecord');
			...
			throw new MFDbException(MF::t('core','The active record cannot be deleted because it is new.'));
	}
	...
	{
		MF::trace(get_class($this).'.refresh()','system.db.ar.MFActiveRecord');
	 ...
	 * @param MFActiveRecord $record record to compare to
	 ...
	 * @param MFDbCriteria $criteria the query criteria. This parameter may be modified by merging {@link dbCriteria}.
	 */
	 ...
	 * If an array, it is treated as the initial values for constructing a {@link MFDbCriteria} object;
	 * Otherwise, it should be an instance of {@link MFDbCriteria}.
	 * @param array $params parameters to be bound to an SQL statement.
	 * This is only used when the first parameter is a string (query condition).
	 * In other cases, please use {@link MFDbCriteria::params} to set parameters.
	 * @return static the record found. Null if no record is found.
	 */
	public function find($condition='',$params=array())
	{
		MF::trace(get_class($this).'.find()','system.db.ar.MFActiveRecord');
	...
	{
		MF::trace(get_class($this).'.findAll()','system.db.ar.MFActiveRecord');
		$criteria=$this->getCommandBuilder()->createCriteria($condition,$params);
	...
	{
		MF::trace(get_class($this).'.findByPk()','system.db.ar.MFActiveRecord');
	...
	{
		MF::trace(get_class($this).'.findAllByPk()','system.db.ar.MFActiveRecord');
		$prefix=$this->getTableAlias(true).'.';
		$criteria=$this->getCommandBuilder()->createPkCriteria($this->getTableSchema(),$pk,$condition,$params,$prefix);
		return $this->query($criteria,true);
	}
	...
	{
		MF::trace(get_class($this).'.findByAttributes()','system.db.ar.MFActiveRecord');
		$prefix=$this->getTableAlias(true).'.';
	...
	{
		MF::trace(get_class($this).'.findAllByAttributes()','system.db.ar.MFActiveRecord');
		$prefix=$this->getTableAlias(true).'.';
		$criteria=$this->getCommandBuilder()->createColumnCriteria($this->getTableSchema(),$attributes,$condition,$params,$prefix);
		return $this->query($criteria,true);
	}
	...
	{
		MF::trace(get_class($this).'.findBySql()','system.db.ar.MFActiveRecord');
		$this->beforeFind();
	...
	{
		MF::trace(get_class($this).'.findAllBySql()','system.db.ar.MFActiveRecord');
		$this->beforeFind();
		if(($criteria=$this->getDbCriteria(false))!==null && !empty($criteria->with))
		{
			$this->resetScope(false);
	...
	{
		MF::trace(get_class($this).'.count()','system.db.ar.MFActiveRecord');
		$this->beforeCount();
		$builder=$this->getCommandBuilder();
	...
	{
		MF::trace(get_class($this).'.countByAttributes()','system.db.ar.MFActiveRecord');
		$prefix=$this->getTableAlias(true).'.';
		$builder=$this->getCommandBuilder();
		$this->beforeCount();
	...
	{
		MF::trace(get_class($this).'.countBySql()','system.db.ar.MFActiveRecord');
		$this->beforeCount();
	...
	{
		MF::trace(get_class($this).'.exists()','system.db.ar.MFActiveRecord');
		$builder=$this->getCommandBuilder();
		$criteria=$builder->createCriteria($condition,$params);
		$table=$this->getTableSchema();
		$criteria->select='1';
	...
	/**
	 * Sets {@link MFDbCriteria::together} property to be true.
	 * This is only used in relational AR query. Please refer to {@link MFDbCriteria::together}
	 * for more details.
	 * @return static the AR object itself
	 * @since 1.1.4
	 */
	...
	{
		MF::trace(get_class($this).'.updateByPk()','system.db.ar.MFActiveRecord');
		$builder=$this->getCommandBuilder();
		$table=$this->getTableSchema();
	...
	{
		MF::trace(get_class($this).'.updateAll()','system.db.ar.MFActiveRecord');
		$builder=$this->getCommandBuilder();
		$criteria=$builder->createCriteria($condition,$params);
	...
	{
		MF::trace(get_class($this).'.updateCounters()','system.db.ar.MFActiveRecord');
		$builder=$this->getCommandBuilder();
		$criteria=$builder->createCriteria($condition,$params);
		$command=$builder->createUpdateCounterCommand($this->getTableSchema(),$counters,$criteria);
		return $command->execute();
	...
	{
		MF::trace(get_class($this).'.deleteByPk()','system.db.ar.MFActiveRecord');
		$builder=$this->getCommandBuilder();
	...
	{
		MF::trace(get_class($this).'.deleteAll()','system.db.ar.MFActiveRecord');
		$builder=$this->getCommandBuilder();
		$criteria=$builder->createCriteria($condition,$params);
		$command=$builder->createDeleteCommand($this->getTableSchema(),$criteria);
		return $command->execute();
	...
	{
		MF::trace(get_class($this).'.deleteAllByAttributes()','system.db.ar.MFActiveRecord');
		$builder=$this->getCommandBuilder();
		$table=$this->getTableSchema();
...
/**
 * MFBaseActiveRelation is the base class for all active relations.
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db.ar
 */
class MFBaseActiveRelation extends MFComponent
{
	/**
	...
	public $select='*';
	/**
	 * @var string WHERE clause. For {@link MFActiveRelation} descendant classes, column names
	...
	public $params=array();
	/**
	 * @var string GROUP BY clause. For {@link MFActiveRelation} descendant classes, column names
	...
	public $joinOptions='';
	/**
	 * @var string HAVING clause. For {@link MFActiveRelation} descendant classes, column names
	 * referenced in this property should be disambiguated with prefix 'relationName.'.
	 */
	public $having='';
	/**
	 * @var string ORDER BY clause. For {@link MFActiveRelation} descendant classes, column names
	 * referenced in this property should be disambiguated with prefix 'relationName.'.
	 */
	public $order='';
	...
	{
		if($criteria instanceof MFDbCriteria)
...
/**
 * MFStatRelation represents a statistical relational query.
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db.ar
 */
class MFStatRelation extends MFBaseActiveRelation
{
	/**
...
/**
 * MFActiveRelation is the base class for representing active relations that bring back related objects.
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db.ar
 * @since 1.0
 */
class MFActiveRelation extends MFBaseActiveRelation
{
	/**
	 ...
	 * For more details about this property, see {@link MFActiveRecord::with()}.
	 */
	public $with=array();
	/**
...
/**
 * MFBelongsToRelation represents the parameters specifying a BELONGS_TO relation.
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db.ar
 * @since 1.0
 */
class MFBelongsToRelation extends MFActiveRelation
{
}


/**
 * MFHasOneRelation represents the parameters specifying a HAS_ONE relation.
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db.ar
 * @since 1.0
 */
class MFHasOneRelation extends MFActiveRelation
{
}


/**
 * MFHasManyRelation represents the parameters specifying a HAS_MANY relation.
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db.ar
 * @since 1.0
 */
class MFHasManyRelation extends MFActiveRelation
{
	/**
...
/**
 * MFManyManyRelation represents the parameters specifying a MANY_MANY relation.
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db.ar
 * @since 1.0
 */
class MFManyManyRelation extends MFHasManyRelation
{
	/**
	 ...
	 * @throws MFDbException if {@link foreignKey} has been specified in wrong format.
	 */
	private function initJunctionData()
	{
		if(!preg_match('/^\s*(.*?)\((.*)\)\s*$/',$this->foreignKey,$matches))
			throw new MFDbException(MF::t('core','The relation "{relation}" in active record class "{class}" is specified with an invalid foreign key. The format of the foreign key must be "joinTable(fk1,fk2,...)".',
...
/**
 * MFActiveRecordMetaData represents the meta-data for an Active Record class.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db.ar
 * @since 1.0
 */
class MFActiveRecordMetaData
{
	/**
	...
	/**
	 * Constructor.
	 * @param MFActiveRecord $model the model instance
	 * @throws MFDbException if specified table for active record class cannot be found in the database
	 */
	public function __construct($model)
	{
		...
		if(($table=$model->getDbConnection()->getSchema()->getTable($tableName))===null)
			throw new MFDbException(MF::t('core','The table "{table}" for active record class "{class}" cannot be found in the database.',
	...
	public function addRelation($name,$config)
	{
		if(isset($config[0],$config[1],$config[2]))  // relation class, AR class, FK
			...
			throw new MFDbException(MF::t('core','Active record "{class}" has an invalid configuration for relation "{relation}". It must specify the relation type, the related active record class and the foreign key.', array('{class}'=>$this->_modelClassName,'{relation}'=>$name)));
	}
