...
/**
 * MFActiveFinder implements eager loading and lazy loading of related active records.
 *
 ...
 * @since 1.0
 */
class MFActiveFinder extends MFComponent
{
	/**
	 ...
	 * @param MFActiveRecord $model the model that initiates the active finding process
	...
	public function __construct($model,$with)
	{
		...
		$this->_joinTree=new MFJoinElement($this,$model);
	 ...
	 * @param MFDbCriteria $criteria the DB criteria
	 ...
	 * @return MFActiveRecord
	 */
	public function findBySql($sql,$params=array())
	{
		MF::trace(get_class($this->_joinTree->model).'.findBySql() eagerly','system.db.ar.MFActiveRecord');
	...
	{
		MF::trace(get_class($this->_joinTree->model).'.findAllBySql() eagerly','system.db.ar.MFActiveRecord');
	...
	{
		MF::trace(get_class($this->_joinTree->model).'.count() eagerly','system.db.ar.MFActiveRecord');
		$this->joinAll=$criteria->together!==true;
	 ...
	 * This method is internally invoked by {@link MFActiveRecord} to support lazy loading.
	 * @param MFActiveRecord $baseRecord the base record whose related objects are to be loaded
	 */
	 ...
	 * @return MFActiveRecord active record model instance
	 *
	 * @since 1.1.14
	 */
	public function getModel($className)
	{
		return MFActiveRecord::model($className);
	}
	 ...
	 * @param MFJoinElement $parent the parent tree node
	 ...
	 * @throws MFDbException if given parent tree node is an instance of {@link MFStatElement}
	...
	{
		if($parent instanceof MFStatElement)
			throw new MFDbException(MF::t('core','The STAT relation "{name}" cannot have child relations.',
		...
		if(is_string($with))
		{
			...
			if(($relation=$parent->model->getActiveRelation($with))===null)
				throw new MFDbException(MF::t('core','Relation "{name}" is not defined in active record class "{class}".',
					array('{class}'=>get_class($parent->model), '{name}'=>$with)));

			$relation=clone $relation;
			$model=$this->getModel($relation->className);

			if($relation instanceof MFActiveRelation)
			{
			...
			if($relation instanceof MFStatRelation)
				$criteria->select='*';
			...
			if($relation instanceof MFStatRelation)
				return new MFStatElement($this,$relation,$parent);
			else
			{
				if(isset($parent->children[$with]))
				{
					...
					$element=new MFJoinElement($this,$relation,$parent,++$this->_joinCount);
...
/**
 * MFJoinElement represents a tree node in the join tree created by {@link MFActiveFinder}.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db.ar
 * @since 1.0
...
class MFJoinElement
{
	...
	public $id;
	/**
	 * @var MFActiveRelation the relation represented by this tree node
	 */
	public $relation;
	/**
	 * @var MFActiveRelation the master relation
	 */
	public $master;
	/**
	 * @var MFActiveRelation the slave relation
	 */
	public $slave;
	/**
	 * @var MFActiveRecord the model associated with this tree node
	 */
	public $model;
	/**
	...
	/**
	 * Constructor.
	 * @param MFActiveFinder $finder the finder
	...
	public function find($criteria=null)
	{
		...
		{
			$query=new MFJoinQuery($this,$criteria);
		...
		{
			$query=new MFJoinQuery($this->_parent);
			$this->_joined=true;
			$query->join($this);
			$this->buildQuery($query);
	 ...
	 * @param MFActiveRecord $baseRecord the active record whose related object is to be fetched.
	 */
	public function lazyFind($baseRecord)
	{
		...
		if(empty($child->records))
			return;
		if($child->relation instanceof MFHasOneRelation || $child->relation instanceof MFBelongsToRelation)
	 ...
	 * @param MFJoinQuery $query represents a JOIN SQL statements
	 * @param MFActiveRecord $record the active record whose related object is to be fetched.
	 * @throws MFDbException if relation in active record class is not specified correctly
	 */
	private function applyLazyCondition($query,$record)
	{
		$schema=$this->_builder->getSchema();
		$parent=$this->_parent;
		if($this->relation instanceof MFManyManyRelation)
		{
			$query->conditions=array(
			...
			if(($joinTable=$schema->getTable($joinTableName))===null)
				throw new MFDbException(MF::t('core','The relation "{relation}" in active record class "{class}" is not specified correctly: the join table "{joinTable}" given in the foreign key cannot be found in the database.',
			...
			if($parentCondition!==array() && $childCondition!==array())
			{
				...
				throw new MFDbException(MF::t('core','The relation "{relation}" in active record class "{class}" is specified with an incomplete foreign key. The foreign key must consist of columns referencing both joining tables.',
		...
		else
		{
			...
			foreach($fks as $i=>$fk)
			{
				...
				if($element->relation instanceof MFBelongsToRelation)
				{
					if(is_int($i))
					{
	...
	/**
	 * Calls {@link MFActiveRecord::afterFind} of all the records.
	 */
	 ...
	 * @param MFJoinQuery $query the query being built up
	 */
	public function buildQuery($query)
	{
		foreach($this->children as $child)
		{
			if($child->master!==null)
				$child->_joined=true;
			elseif($child->relation instanceof MFHasOneRelation || $child->relation instanceof MFBelongsToRelation
	 ...
	 * @param MFJoinQuery $query the query to be executed.
	 */
	 ...
	 * @param MFJoinQuery $query the query executed
	 * @param array $row a row of data
	 * @return MFActiveRecord the populated record
	 */
	private function populateRecord($query,$row)
	{
		...
		foreach($this->children as $child)
		{
			...
			else // has_many and many_many
			{
				...
				if($childRecord instanceof MFActiveRecord)
					$fpk=serialize($childRecord->getPrimaryKey());
				else
					$fpk=0;
				if(!isset($this->_related[$pk][$child->relation->name][$fpk]))
				{
					if($childRecord instanceof MFActiveRecord && $child->relation->index!==null)
	 ...
	 * @throws MFDbException if active record class is trying to select an invalid column
		...
		else
		{
			...
			foreach($select as $name)
			{
				...
				elseif(preg_match('/^(.*?)\s+AS\s+(\w+)$/im',$name,$matches)) // if the column is already aliased
				{
					...
					throw new MFDbException(MF::t('core','Active record "{class}" is trying to select an invalid column "{column}". Note, the column must exist in the table or be an expression with alias.',
	...
	public function getJoinCondition()
	{
		...
		else
		{
			...
			if($this->slave!==null)
			{
				if($this->relation instanceof MFBelongsToRelation)
				{
					$fks=array_flip($fks);
					$pke=$this->slave;
					$fke=$this;
			...
			elseif($this->relation instanceof MFBelongsToRelation)
			{
				$pke=$this;
				$fke=$parent;
			}
	 ...
	 * @param MFJoinElement $fke the join element containing foreign keys
	 * @param array $fks the foreign keys
	 * @param MFJoinElement $pke the join element contains primary keys
	 * @param MFJoinElement $parent the parent join element
	 * @return string the join statement
	 * @throws MFDbException if a foreign key is invalid
	 */
	private function joinOneMany($fke,$fks,$pke,$parent)
	{
		...
		foreach($fks as $i=>$fk)
		{
			...
			if(!isset($fke->_table->columns[$fk]))
				throw new MFDbException(MF::t('core','The relation "{relation}" in active record class "{class}" is specified with an invalid foreign key "{key}". There is no such column in the table "{table}".',
	 ...
	 * @param MFDbTableSchema $joinTable the join table
	 * @param array $fks the foreign keys
	 * @param MFJoinElement $parent the parent join element
	 * @return string the join statement
	 * @throws MFDbException if a foreign key is invalid
...
/**
 * MFJoinQuery represents a JOIN SQL statement.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db.ar
 * @since 1.0
...
class MFJoinQuery
{
	...
	/**
	 * Constructor.
	 * @param MFJoinElement $joinElement The root join tree.
	 * @param MFDbCriteria $criteria the query criteria
	 */
	 ...
	 * @param MFJoinElement $element the element to be joined
	 */
	 ...
	 * @param MFDbCommandBuilder $builder the command builder
	 * @return MFDbCommand DB command instance representing the SQL statement
	 */
...
/**
 * MFStatElement represents STAT join element for {@link MFActiveFinder}.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db.ar
 */
class MFStatElement
{
	 ...
	 * Constructor.
	 * @param MFActiveFinder $finder the finder
	 * @param MFStatRelation $relation the STAT relation
	 * @param MFJoinElement $parent the join element owning this STAT element
	 */
	...
	private function queryOneMany()
	{
		...
		if(count($fks)!==count($pkTable->primaryKey))
			throw new MFDbException(MF::t('core','The relation "{relation}" in active record class "{class}" is specified with an invalid foreign key. The columns in the key must match the primary keys of the table "{table}".',
		...
		foreach($fks as $i=>$fk)
		{
			...
			if(isset($table->foreignKeys[$fk]))
			{
				...
				if($schema->compareTableNames($pkTable->rawName,$tableName))
					...
					throw new MFDbException(MF::t('core','The relation "{relation}" in active record class "{class}" is specified with a foreign key "{key}" that does not point to the parent table "{table}".',
