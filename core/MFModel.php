...
/**
 * MFModel is the base class providing the common features needed by data model objects.
 *
 * MFModel defines the basic framework for data models that need to be validated.
 *
 * @property MFList $validatorList All the validators declared in the model.
 ...
 * @property MFMapIterator $iterator An iterator for traversing the items in the list.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.base
 * @since 1.0
 */
abstract class MFModel extends MFComponent implements IteratorAggregate, ArrayAccess
{
	 ...
	 * </pre>
	 *   A built-in validator refers to one of the validators declared in {@link MFValidator::builtInValidators}.
	 *   And a validator class is a class extending {@link MFValidator}.</li>
	...
	protected function afterConstruct()
	{
		if($this->hasEventHandler('onAfterConstruct'))
			$this->onAfterConstruct(new MFEvent($this));
	}
	...
	{
		$this->onAfterValidate(new MFEvent($this));
	}
	 ...
	 * Also, since this method return a {@link MFList} object, you may
	 ...
	 * The change made to the {@link MFList} object will persist and reflect
	 * in the result of the next call of {@link getValidators}.
	 * @return MFList all the validators declared in the model.
	 * @since 1.1.2
	 */
	 ...
	 * @throws Exception if current class has an invalid validation rule
	 * @return MFList validators built based on {@link rules()}.
	 */
	public function createValidators()
	{
		$validators=new MFList;
		foreach($this->rules() as $rule)
		{
			if(isset($rule[0],$rule[1]))  // attributes, validator name
				$validators->add(MFValidator::createValidator($rule[1],$this,$rule[0],array_slice($rule,2)));
			else
				throw new Exception(MF::t('core','{class} has an invalid validation rule. The rule must specify attributes to be validated and the validator name.',
	 ...
	 * {@link MFRequiredValidator} validation rule in the current {@link scenario}.
	...
	public function isAttributeRequired($attribute)
	{
		...
		{
			if($validator instanceof MFRequiredValidator)
				return true;
		}
		return false;
	}
	 ...
	 * The default implementation will log a warning message if MF_DEBUG is on.
	...
	{
		if(MF_DEBUG)
			MF::log(MF::t('core','Failed to set unsafe attribute "{attribute}" of "{class}".',array('{attribute}'=>$name, '{class}'=>get_class($this))),CLogger::LEVEL_WARNING);
	}
	 ...
	 * @return MFMapIterator an iterator for traversing the items in the list.
	 */
	public function getIterator()
	{
		$attributes=$this->getAttributes();
		return new MFMapIterator($attributes);
	}
