...
class MFSecurity extends MFService
{
	const STATE_VALIDATION_KEY = 'MF.MFSecurity.validationkey';
	const STATE_ENCRYPTION_KEY = 'MF.MFSecurity.encryptionkey';
	...
	*/
	...
	public function getValidationKey()
	{
	    ...
	    {
	        $key = MF::app()->states->get(self::STATE_VALIDATION_KEY);
	        if($key !== null)
	            $this->setEncryptionKey($key);
	        else
	        {
	            $key = $this->generateRandomString(32, true);
	            if($key === false)
	            {
	                $key = $this->generateRandomString(32, false);
	                if($key === false)
	                    throw new Exception(MF::t('core',
	                        'MFSecurity::generateRandomString() cannot generate random string in the current environment.'));
	            }
	            $this->setValidationKey($key);
	            MF::app()->states->set(self::STATE_VALIDATION_KEY, $key);
	        }
	        return $this->_validationKey;
	    }
	}
	...
	public function setValidationKey($value)
	{
	    if(!empty($value))
	        ...
	        throw new Exception(MF::t('core','MFSecurity.validationKey cannot be empty.'));
	}
	...
	public function getEncryptionKey()
	{
	    ...
	    {
	        $key = MF::app()->states->get(self::STATE_ENCRYPTION_KEY);
	        if($key !== null)
	            $this->setEncryptionKey($key);
	        else
	        {
	            ...
	            $this->setEncryptionKey($key);
	            MF::app()->states->set(self::STATE_ENCRYPTION_KEY, $key);
	        }
	        return $this->_encryptionKey;
	    }
	}
	 ...
	 * @throws Exception if PHP Mcrypt extension is not loaded or key is invalid
	 */
	...
	protected function openCryptModule()
	{
	    if(extension_loaded('mcrypt'))
	    {
	        ...
	        if($module===false)
	            throw new Exception(MF::t('core','Failed to initialize the mcrypt module.'));
	
	        return $module;
	    }
	    else
	        throw new Exception(MF::t('core','MFSecurity requires PHP mcrypt extension to be loaded in order to use data encryption feature.'));
	}
	...
	public function computeHMAC($data,$key=null,$hashAlgorithm=null)
	{
	    ...
	    {
	        throw new Exception(MF::t('core','Only SHA1 and MD5 hashing algorithms are supported when using PHP 5.1.1 or below.'));
	    }
	 ...
	 * @throws Exception if the supported key lengths of the cipher are unknown
	 */
	    ...
	    if(is_string($key))
	    {
	        ...
	        if($supportedKeyLengths)
	        {
	            if(!in_array($this->strlen($key),$supportedKeyLengths)) {
	                throw new Exception(MF::t('core','Encryption key length can be {keyLengths}.',array('{keyLengths}'=>implode(',',$supportedKeyLengths))));
	            }
	        }
	        elseif(isset(self::$encryptionKeyMinimumLengths[$cryptAlgorithm]))
	        {
	            ...
	            if($this->strlen($key)<$minLength || $this->strlen($key)>$maxLength)
	                throw new Exception(MF::t('core','Encryption key length must be between {minLength} and {maxLength}.',array('{minLength}'=>$minLength,'{maxLength}'=>$maxLength)));
	        }
	        else
	            throw new Exception(MF::t('core','Failed to validate key. Supported key lengths of cipher not known.'));
	    }
	    else
	        throw new Exception(MF::t('core','Encryption key should be a string.'));
	}
	...
	public function legacyDecrypt($data,$key=null,$cipher='des')
	{
	    if(!$key)
	    {
	        $key=MF::app()->states->get(self::STATE_ENCRYPTION_KEY);
	        if(!$key)
	            throw new Exception(MF::t('core','No encryption key specified.'));
	        $key = md5($key);
	    }
