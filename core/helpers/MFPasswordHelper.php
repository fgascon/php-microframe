...
/**
 * MFPasswordHelper provides a simple API for secure password hashing and verification.
 *
 * MFPasswordHelper uses the Blowfish hash algorithm available in many PHP runtime
 ...
 * MFPasswordHelper fails when run in an environment that does not have
 ...
 *
 * MFPasswordHelper throws an exception if the Blowfish hash algorithm is not
 * available in the runtime PHP's crypt() function. It can be used as follows
 *
 * Generate a hash from a password:
 * <pre>
 * $hash = MFPasswordHelper::hashPassword($password);
 * </pre>
 ...
 * <pre>
 * if (MFPasswordHelper::verifyPassword($password, $hash))
 *     // password is good
 * else
 *     // password is bad
 * </pre>
...
class MFPasswordHelper
{
	...
	protected static function checkBlowfish()
	{
		if(!function_exists('crypt'))
			throw new Exception(MF::t('core','{class} requires the PHP crypt() function. This system does not have it.',
	...
	public static function hashPassword($password,$cost=13)
	{
		...
		if(!is_string($hash) || (function_exists('mb_strlen') ? mb_strlen($hash, '8bit') : strlen($hash))<32)
			throw new Exception(MF::t('core','Internal error while generating hash.'));
	...
	{
		if(!is_numeric($cost))
			throw new Exception(MF::t('core','{class}::$cost must be a number.',array('{class}'=>__CLASS__)));

		$cost=(int)$cost;
		if($cost<4 || $cost>31)
		    throw new Exception(MF::t('core','{class}::$cost must be between 4 and 31.',array('{class}'=>__CLASS__)));

		if(($random=MF::app()->getSecurity()->generateRandomString(22,true))===false)
			if(($random=MF::app()->getSecurity()->generateRandomString(22,false))===false)
				throw new Exception(MF::t('core','Unable to generate random string.'));
