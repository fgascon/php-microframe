...
/**
 * MFList implements an integer-indexed collection class.
 *
 ...
 * MFList can also be used like a regular array as follows,
 * <pre>
 ...
 * </pre>
 *
 * To extend MFList by doing additional operations with each addition or removal
 ...
 * @since 1.0
 */
class MFList extends MFComponent implements IteratorAggregate,ArrayAccess,Countable
{
	/**
	...
	{
		return new MFListIterator($this->_d);
	}
	...
	public function itemAt($index)
	{
		...
		elseif($index>=0 && $index<$this->_c) // in case the value is null
			...
			throw new Exception(MF::t('core','List index "{index}" is out of bound.',
				array('{index}'=>$index)));
	}
	...
	public function insertAt($index,$item)
	{
		if(!$this->_r)
		{
			...
			throw new Exception(MF::t('core','The list is read only.'));
	}
	...
	public function copyFrom($data)
	{
		if(is_array($data) || ($data instanceof Traversable))
		{
			if($this->_c>0)
				$this->clear();
			if($data instanceof MFList)
				$data=$data->_d;
			foreach($data as $item)
				$this->add($item);
		}
		elseif($data!==null)
			throw new Exception(MF::t('core','List data must be an array or an object implementing Traversable.'));
	}
